...
| NV10 — Every link's purpose is clear from its link text alone or from its programmatic context (Links list: no bare "click here"/"more", no identical texts pointing to different targets) | pass | O16 — reviewer 2026-09-21, confirmed **on focus** this time: no link announces a purpose they could not act on. The one bare-looking text, "view", sits in a cell that reads "Instructor Default view", so its programmatic context carries it. Its real defect is a different row → O17, MO11/2.1.1 |
...
- O16 [clarified] (state: Class Management standard mode, NVDA, reviewer 2026-09-21 — step W72): **"nv10, no"** — no link announces a purpose that could not be acted on. This closes the row that was reopened on 2026-09-21, and it was answered the way the process requires: **on focus**, not from the links-list dialog that misled the 2026-09-14 reading (O13, O15).
  - Classified: NV10 / WCAG 2.4.4 / pass
- O17 [clarified] (state: as O16): the reviewer adds a finding the row itself does not cover — **"I see a 'view' link that is in the Class Assignments Table that is out of the tab order but is reachable via NVDA links view."** Measured the same day and confirmed exactly: the control is `<a class="actionMenu" onclick="LaunchGradeProfile(17547, 'Instructor Default');">view</a>` with **no `href`**, in the assignments grid (`#MainContent_gv2_DXMainTable`), in the cell reading "Instructor Default". A full tab cycle was walked with real dispatched Tab keys — **41 stops, then the order wraps, and the link is never focused**. So a screen-reader user can reach and activate it from the links list, while a sighted keyboard-only user cannot reach it at all. It also sits at x⁈1887, outside the 1300 px container, so it needs horizontal scrolling to see (V-F19).
  - Classified: MO11-adjacent / WCAG 2.1.1, 4.1.2 / → finding **V-F32**
