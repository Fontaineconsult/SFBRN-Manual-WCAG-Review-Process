...
**Not blocked — correction 2026-09-21.** An earlier note here read the reviewer's "there are no grades to view in the test account" as covering this task. They have since clarified: that was the **gradebook**, not the grade report. **View Grade Report (S4) is fully populated** — measurement found a real grade formula (`Student Grade = 100 - 100 - 9 = 0%`), 14 dated submissions and part-level late notices (R100 O5–O7) — so the student half of this task **can and should be walked**: W21 asks about the jump-point density, the per-part submission tables, the blue [?] links and the late convention, and all four have content to read. One of them is already answered: lateness is stated in words at part level and by red date-time alone at submission level (R100 O7 → V-F31). The instructor half (class gradebook) left the sample 2026-09-15 as not student-facing and is not owed. Verdict stays Not run **pending the walk**, not for want of data.
...
| **Evidence** | R109 O5 (S3), R110 O5 (S8), R102 O3 (S6), R099 O4 (S1) — all reviewer, grayscale filter, 2026-09-21; R100 O6, O7, O8 (S4 — the grade formula, the red date-times, and the reviewer's grayscale ruling); R119 O3 (S13) and R105 O3 (S12) — extended by the same-styles ruling after measuring the tokens each view uses (`evidence/colour-token-scan-2026-09-21.json`); element list and colour tokens from R004 O4/O10 and R001 O4 (axe) |
