...
- O1 [new] (state: Student Practice Area, book "Expert TA: Introduction to Physics"): axe `label` (critical) ×12 — Books and Chapters combos, and the difficulty/type filter check boxes' hidden state fields ("All Problems", "1 Easy" … — PW-G) (PW-A).
  - Proposed: W1 / WCAG 1.3.1 + 4.1.2 (A) / Major — add to the unlabeled-editors finding after JAWS confirmation; the check boxes need the keyboard run to establish whether the hidden field or the visible box takes focus
- O2 [new] (state: as O1): `color-contrast` ×4 — "Library" (`#EFBB75`, **1.74:1**), "Books" / "Chapters" / "Filter by Problem Difficulty and Type" (`#3A7C89` on `#EDEDED`, **4.05:1**) — same tokens as the Assignment Editor (R006 O2).
  - Proposed: W1 / WCAG 1.4.3 (AA) / Major — add this view to the contrast finding
- O3 [new] (state: as O1): `aria-command-name` ×1, `aria-hidden-focus` ×1, `region` ×30, `landmark-one-main`, `page-has-heading-one` (PW-B/C/D); passes 28 / inapplicable 56.
  - Proposed: fold into product-wide findings
- O4 [new] (state: as O1): **incomplete** `color-contrast` ×2 — "Take Tutorial Assignment" and "Clear Selection" button captions (PW-F).
...
axe-core 4.10.3, full ruleset, 17:48 UTC. Counts: violations 7, incomplete 2, passes 28, inapplicable 56. Direct URL; no problem selected, nothing started.

## Evidence files in this folder

- `R011-axe.json` — raw axe output (273,562 bytes)
