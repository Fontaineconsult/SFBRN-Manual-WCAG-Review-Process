...
- **Task findings:** V-F12, V-F16 (Minor), V-F34 (Blocker for the grade report's purpose), V-F35 (Major), V-F36 (Major)
- **Remarks:** 2026-09-14 S1: the grids' expand controls are clickable images with no text alternative (V-F16). [provisional] 2026-09-10 NVDA on S3: the Problem 8 figure is hidden from AT (empty alt on an informative diagram; `G` finds no graphic). MathJax math is readable in browse mode (V-F10 withdrawn). Solutions-page figures with broken `alt=` markup (R014) still to be confirmed. Vendor-claim discrepancy. 2026-09-21, View Printable Assignment: 8 of 14 figures carry no usable alternative, and **2 are CSS `background-image` divs** that cannot be given alt text without markup changes — the only structural (not editorial) instance in the review; 4 figures on the same page do carry good descriptions, so the vendor writes them when it writes any (V-F36). 2026-09-21, View Assignment Solutions: 14 of 15 figures have no `alt` at all and NVDA reads the file names aloud; 7 carry a malformed `alt=` swallowed into the `src`, so the alternative text was written and never reaches the browser (V-F35) — the cheapest fix in the review. 2026-09-21, View Grade Report walked with NVDA: 20 images, all inside submission table cells, 16 with no alt — and they are the rendering of the student's own submitted work, so the page's stated purpose ("shows your detailed work") is unavailable without sight (V-F34).
