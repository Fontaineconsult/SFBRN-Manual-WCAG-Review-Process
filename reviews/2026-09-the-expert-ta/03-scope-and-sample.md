...
| Exclusions and justification | **Instructor-facing views are out of the sample (reviewer, 2026-09-15: "cross off all views that are not student facing")** — the product boundary is unchanged, but the sample and the report are scoped to what a student uses: S5 Assignment Editor, S9 Class Assignments Grade Sheet, S10 Manage Class Roster, S11 Edit Class popup, R1 Academic Integrity Preferences and R2 View Grades (spreadsheet) are marked removed in §3.1/§3.2 (their runs and findings stay on record). Consequences for the reviewer to rule on: the random sample (§3.2) is now empty and must be redrawn from student-facing views; task T4 (instructor authoring) and the instructor branches of T3 (grant extension) and T5 (class grade sheet) cannot be walked; findings V-F25 (Edit Class popup at zoom) and V-F26 (Library video captions) sit only on removed views — **reviewer 2026-09-15: keep them** (reported as instructor-facing findings); T4 stays Not run; the random redraw (R3, R4) is still owed. (proposed 2026-09-10 — reviewer decides) `theexpertta.com` marketing and help pages — separate content site; sample only the two support pages reached from the in-app Help menu (A2), because Section 508 Ch. 6 covers support documentation. `reg.theexpertta.com` student self-registration and payment — not exercisable without creating a paid account; record as a coverage limitation, not a pass. **The demo account's *gradebook* holds no data** — clarified by the reviewer 2026-09-21: their "no grades to view in the test account" meant the **class gradebook**, not the student's grade report. The grade report (S4) is fully populated and has been tested (R100: a real grade formula, 14 dated submissions, part-level late notices). The gradebook is the instructor-facing Class Assignments Grade Sheet (S9), already out of the sample as not student-facing, so this costs the review nothing: no student-facing view is untested for want of data. Recorded because an earlier note in this file wrongly generalised it to the grade report. Canvas LTI launch and grade pass-back — blocked until the campus enables the integration, which happens only after this review; coverage limitation. |
...
| C4 | Module / content page | No LMS-style content pages in this product (confirmed 2026-09-10). Nearest read-only content views: View Printable Assignment `https://dei56mo.theexpertta.com/Common/ViewAssignmentDetails.aspx` (sampled as **S13** on 2026-09-15 at the reviewer's request — a plain HTML rendering of the assignment's problems) and View Assignment Solutions `https://dei56mo.theexpertta.com/Common/ViewAssignmentSolutionsV2.aspx` (the only view with a real heading outline: h1 + h2 per problem). The vendor's eReader/eBook is **not provisioned in the demo** (hypothesis — ask vendor). |
...
| C6 | Quiz / exam | Same surface as C5 with dates and deductions (confirmed 2026-09-10). Practice Mode `https://dei56mo.theexpertta.com/Common/TakeAssignmentInPracticeMode.aspx` exists but the sample assignment is not configured for it. Reviewer 2026-09-15: the top-menu "Practice" link lands there — "This assignment is not currently configured to allow practice mode"; not testable in the demo. (S12, the Student Practice Area library browser, is a different page reached by its direct address.) Secure/timed exams and Respondus not provisioned in the demo (hypothesis). |
...
| Content pages | Printable assignment, assignment solutions (h1/h2 outline) | S8 (confirmed 2026-09-10); S13 printable assignment (added 2026-09-15) |
...
| Embedded media players | **YouTube embeds** in the expanding area under **Library** on the Assignment Editor — discovered by the reviewer 2026-09-15 ("they have captions and are YouTube embeds … they are auto generated"; R037 O6, V-F26). The probe's media sweep (14 views, 2026-09-11) saw none because it measures the view as loaded — the embeds sit in a panel opened on demand. Vendor claims captions/transcripts/audio description for its video library — captions present but auto-generated; audio description / transcript still a hypothesis (NV11 on R035). |
...
| S5 | Assignment Editor (edit) — proposed 2026-09-10 — removed 2026-09-15: not student-facing (reviewer) | `https://dei56mo.theexpertta.com/Common/AssignmentEditor.aspx?m=2&eid=3373&aid=17547` | C11; F7; library browser, "Accessibility(AA) Only" filter, Extensions/Messages (A4/A5 candidates) |
...
| S9 | Class Assignments Grade Sheet — proposed 2026-09-10 — removed 2026-09-15: not student-facing (reviewer) | `https://dei56mo.theexpertta.com/Common/GradeSheetClassAssignments.aspx?m=1&eid=3373` | C7 (instructor); F6; data tables, export controls |
| S10 | Manage Class Roster — proposed 2026-09-10 — removed 2026-09-15: not student-facing (reviewer) | `https://dei56mo.theexpertta.com/Common/vwMates.aspx?m=1&eid=3373` | C12; F6; A4 candidate; data table |
| S11 | Edit Class popup — proposed 2026-09-10 — removed 2026-09-15: not student-facing (reviewer) | `UI: Class Management → Class Menu → Edit Class → Go` (iframe `/Common/eClass.aspx?m=2&eid=3373`) | C16; popup iframe/modal semantics, form fields |
...
| S13 | View Printable Assignment — "Chapter 5 Sample Assignment", plain HTML rendering of the problems — proposed 2026-09-15 (reviewer: "make sure this is recorded as an available view for the assignments") | `https://dei56mo.theexpertta.com/Common/ViewAssignmentDetails.aspx` via `UI: Class Management → assignment row → View Printable Assignment` | C4; alternate read-only presentation of S3's problems (candidate conforming alternate for reading — not for answering); MathJax, the same figure images as S3 |
| S14 | Password reset — proposed 2026-09-21 (the reviewer walked it with NVDA that day; it was already on record as part of the authentication surface, §1 C1 and A3, but had never been sampled) | `https://login.theexpertta.com/ResetPassword.aspx` via `UI: Sign in → "Trouble Logging in?"` — signed-out window (port 9223) only | C1; A3 authentication; the only account-recovery path in the demo, so a student locked out cannot reach any other view without it |
...
|----|---------------|-----------------|
| R1 | Academic Integrity Preferences — proposed 2026-09-10 — removed 2026-09-15: not student-facing (reviewer) | `https://dei56mo.theexpertta.com/Instructor/AcademicIntegrityTemplates.aspx` |
| R2 | View Grades (Spreadsheet) — proposed 2026-09-10 — removed 2026-09-15: not student-facing (reviewer) | `https://dei56mo.theexpertta.com/Common/GradeSheetClassAssignmentProblems.aspx?m=1&eid=3373&aid=17547` |

**Redraw needed (2026-09-15):** both random samples are instructor-facing and were removed with the instructor views; draw two new random views from the student-facing pool and add them as R3, R4 (IDs are never reused).
