...
| **Result** | Not set — NC1, NC3 need the reviewer — judge from `R111-grayscale.png` (achromatopsia emulation) or the OS filter; measured fail on NC2 awaits confirmation |
...
| NC2 — Links are distinguishable from surrounding text without color | fail | O2 — 1 of 4 link(s) in running text differ from surrounding text by colour only (no underline, border, weight or style change) — measured: Tab for Class News, Enter to skip → javascript:skipper('Feedback'); |
